...
const KanbanCard = ({ job, isDragging = false }) => {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className="rounded-xl border bg-slate-900 p-3 touch-none select-none"
    >
      <p className="text-[0.6rem] font-bold tracking-widest text-slate-500">{job.code}</p>
      <p className="mt-0.5 text-[0.8rem] font-semibold leading-tight text-slate-200 line-clamp-2">
        {job.titulo}
      </p>
      {job.cliente && <p className="mt-1 text-[0.65rem] text-slate-500 truncate">{job.cliente}</p>}
      {job.operario_name && (
        <div className="mt-1.5 flex items-center gap-1">
          <i className="bx bx-user text-[0.7rem] text-slate-600" />
          <span className="text-[0.65rem] text-slate-400">{job.operario_name}</span>
        </div>
      )}
    </div>
  )
...
const DragGhost = ({ job }) => (
  <div
    className="w-[200px] rotate-1 rounded-xl border bg-slate-900 p-3 shadow-2xl"
  ...
  return (
    <div className="flex min-w-[200px] flex-1 flex-col gap-2">
      ...
      <div className="flex items-center gap-2 px-1">
        ...
        <span className="text-[0.65rem] font-bold uppercase tracking-[0.15em] text-slate-400">
          {col.label}
        </span>
        <span
          className="ml-auto rounded-full px-1.5 py-0.5 text-[0.6rem] font-extrabold"
      ...
      >
        ...
        {jobs.length === 0 && (
          <p className="py-4 text-center text-[0.65rem] text-slate-600">Vacío</p>
        )}
      </div>
    </div>
  )
