  ...
  return (
    <article className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
      ...
      <div className="mt-2 h-1.5 w-full rounded-full bg-slate-800">
        <div
          className={cx('h-1.5 rounded-full transition-all', barClass)}
        ...
        >
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-slate-400">
              ...
              <span className="font-bold text-slate-500">{result.omitidas}</span> omitidas (ya
              existían)
            </p>
          </div>
          <button
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3 space-y-2">
            <p className="text-[0.64rem] font-bold uppercase tracking-[0.14em] text-slate-500">
              Atributos
            </p>
            {attrs.map((attr, i) => (
              <div key={i} className="flex gap-2">
                ...
                <input
                  type="text"
                  placeholder="valores separados por coma (ej: M6,M8,M10)"
                  value={attr.values}
          ...
          {totalVariants() > 500 && (
            <p className="text-xs text-rose-400">
              Máximo 500 variantes por generación. Divide en varias llamadas.
            </p>
          )}
          {error && <p className="text-xs font-semibold text-rose-400">{error}</p>}
        </div>
    ...
    <AppShell>
      <div className="space-y-4 pb-8">
        ...
        {!isLoading && !error && filtered.length > 0 && (
          <div className="space-y-2">
            {filtered.map((item) => (
              <MaterialRow key={item.id} item={item} isAdmin={isAdmin} onDelete={handleDelete} />
            ))}
          </div>
        )}
      </div>
