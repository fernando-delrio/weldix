...
const ESTADO_CONFIG = {
  ...
  aprobada:   { label: 'Aprobada',   badge: 'border-emerald-700/50 bg-emerald-500/10 text-emerald-300' },
...
const inputBase = 'w-full rounded-lg border border-slate-700 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none focus:border-sky-500 placeholder:text-slate-600'
const labelBase = 'block text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-slate-500 mb-1'
...
const buildGrid = (year, month) => {
  ...
  return Array.from({ length: total }, (_, i) => {
    ...
    return day >= 1 && day <= daysInMonth
      ? toDateStr(new Date(year, month, day))
      : null
  })
}
...
const RangeCalendar = ({ inicio, fin, onRangeChange }) => {
  ...
  const prevMes = () => setMes(({ year, month }) =>
    month === 0 ? { year: year - 1, month: 11 } : { year, month: month - 1 }
  )
  const nextMes = () => setMes(({ year, month }) =>
  ...
  const cellClass = (dateStr) => {
    ...
    return cx(
      ...
      !isPast && !isEndpoint && !inRange && !isHoverFin && !isWeekend && 'hover:bg-slate-700 text-slate-300',
  ...
  return (
    <div className="select-none">
      ...
      <div className="mb-3 flex items-center justify-between">
        <button type="button" onClick={prevMes} className="grid h-7 w-7 place-items-center rounded-lg border border-slate-700 text-slate-400 hover:text-slate-200 transition">‹</button>
        <span className="text-sm font-bold text-slate-100">
          {MESES[mes.month]} {mes.year}
        </span>
        <button type="button" onClick={nextMes} className="grid h-7 w-7 place-items-center rounded-lg border border-slate-700 text-slate-400 hover:text-slate-200 transition">›</button>
      </div>

      {/* Cabecera días semana */}
      <div className="mb-1 grid grid-cols-7">
        {DIAS_SEMANA.map((d) => (
          <div key={d} className="text-center text-[0.6rem] font-bold uppercase tracking-wider text-slate-600">{d}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7">
        {grid.map((dateStr, idx) => (
          <div
            key={idx}
            className={wrapClass(dateStr, idx)}
          >
            {dateStr ? (
              <div
...
const ContadorDias = ({ inicio, fin, saldo }) => {
  ...
  return (
    <div className={cx(
      ...
      noHayRango   ? 'border-slate-800 bg-slate-800/30'
      : restantes !== null && restantes < 0 ? 'border-rose-800/50 bg-rose-500/5'
      ...
      {noHayRango ? (
        ...
        <div className="flex items-center justify-around">
          <div className="text-center">
            <p className={cx('text-2xl font-bold', restantes !== null && restantes < 0 ? 'text-rose-400' : 'text-sky-300')}>{diasSel}</p>
          ...
          {saldo && (
            <>
              ...
              <div className="text-center">
                <p className={cx('text-2xl font-bold', restantes < 0 ? 'text-rose-400' : 'text-emerald-400')}>{restantes}</p>
                <p className="text-[0.6rem] uppercase tracking-wider text-slate-500">Días restantes</p>
              </div>
            </>
          )}
        </div>
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-[0.68rem] text-slate-400 mt-0.5">
            ...
            <span className="ml-2 text-slate-500">({dias} día{dias !== 1 ? 's' : ''})</span>
          </p>
        </div>
        <span className={cx('shrink-0 rounded-md border px-2 py-0.5 text-[0.6rem] font-bold uppercase tracking-widest', cfg.badge)}>
          {cfg.label}
        </span>
      </div>

      {solicitud.motivo && (
        <p className="text-xs text-slate-500 italic">"{solicitud.motivo}"</p>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative z-10 w-full max-w-[480px] max-h-[90vh] overflow-y-auto rounded-t-2xl border border-slate-700/80 bg-slate-900 p-5 shadow-2xl sm:rounded-2xl">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-700 text-slate-400 hover:text-slate-200">✕</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Motivo */}
          <div>
            <label className={labelBase}>Motivo <span className="text-slate-600 normal-case">(opcional)</span></label>
            <textarea
              value={motivo}
          ...
          {(error || sinSaldo) && (
            <p className="text-xs text-rose-400">
              {sinSaldo ? `No tienes días suficientes. Disponibles: ${saldo.dias_disponibles}` : error}
            </p>
          )}
          ...
          >
            {isSubmitting ? 'Enviando...' : isValid ? `Enviar solicitud (${diasSel}d)` : 'Selecciona las fechas'}
          </button>
        </form>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative z-10 w-full max-w-[480px] rounded-t-2xl border border-slate-700/80 bg-slate-900 p-5 shadow-2xl sm:rounded-2xl">
        ...
        <div className={cx(cardBase, 'mb-4')}>
          ...
          <p className="text-xs text-slate-400">{solicitud.fecha_inicio} → {solicitud.fecha_fin} · {solicitud.dias_solicitados} días</p>
          {solicitud.motivo && <p className="text-xs text-slate-500 italic mt-1">"{solicitud.motivo}"</p>}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className={labelBase}>Comentario <span className="text-slate-600 normal-case">(opcional)</span></label>
            <textarea value={form.comentario_admin} onChange={setField('comentario_admin')} rows={2} maxLength={500} placeholder="Razón de la decisión..." className={cx(inputBase, 'resize-none')} />
          </div>
          {error && <p className="text-xs text-rose-400">{error}</p>}
          <button type="submit" disabled={isSubmitting} className="h-11 w-full rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 text-sm font-bold text-white transition hover:from-sky-400 hover:to-blue-500 disabled:opacity-60">
...
const loadingState = ({ isLoading }) =>
  isLoading && (
    <div className="space-y-3">
      {[1, 2, 3].map((n) => <div key={n} className={cx(cardBase, 'h-20 animate-pulse')} />)}
    </div>
  )

const errorState = ({ error }) =>
  error && <p className="rounded-lg border border-rose-800/50 bg-rose-500/10 p-3 text-sm text-rose-400">{error}</p>
...
const RrhhPage = () => {
  ...
  const {
    saldo, solicitudes, tipos, pendientes,
    isLoading, isSubmitting, error,
    modalCrear, setModalCrear,
    revisandoId, setRevisandoId,
    crear, cancelar, revisar,
  } = useRrhh({ isAdmin })
  ...
  return (
    <AppShell>
      <div className="mx-auto w-full max-w-[720px] space-y-4 pb-5">
        ...
        {!isLoading && solicitudesFiltradas.map((s) => (
          <SolicitudCard
            key={s.id}
            solicitud={s}
            isAdmin={isAdmin}
