...
const T = {
  dark: {
    ...
    btnWhatsapp:  'border border-emerald-700/60 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20',
    footerBg:     'bg-[#040608]',
  ...
  light: {
    ...
    btnWhatsapp:  'border border-emerald-700/60 bg-emerald-500/10 text-emerald-900 hover:bg-emerald-500/20',
    footerBg:     'bg-[#e8e0d4]',
...
const TIMELINE_STEPS = [
  { when: 'Hoy',            icon: 'bx bx-store',        title: 'Te registras',         desc: '30 segundos. Sin tarjeta. Sin instalar nada.' },
  { when: 'En 5 minutos',   icon: 'bx bx-list-check',   title: 'Primera OT creada',    desc: 'Código automático, cliente, asignación al operario.' },
  { when: 'Esta semana',    icon: 'bx bx-time-five',    title: 'Todo el equipo ficha',  desc: 'Desde el móvil, sin App Store. Cumplimiento legal.' },
  { when: 'Este mes',       icon: 'bx bx-bar-chart-alt-2', title: 'Cero papel',         desc: 'Stock, RRHH, equipos y OTs — todo en una pantalla.' },
]

const TESTIMONIALS = [
  {
    quote: 'Antes tardaba media hora al final del día en apuntar las horas y las OTs. Ahora lo hago en tiempo real desde el móvil y el jefe lo ve al instante. Es una pasada.',
    name: 'Miguel Ángel R.',
    role: 'Soldador oficial — Taller Metálicas Norte',
    initials: 'MR',
  },
  {
    quote: 'La inspección de trabajo nos pidió el registro de jornada y en 30 segundos lo tuvimos exportado. Con el Excel anterior hubiera tardado dos días. Ya no hay vuelta atrás.',
    name: 'Carlos F.',
...
const NavBar = ({ isDark, onToggle, t }) => (
  <nav className={cx('fixed inset-x-0 top-0 z-50 flex items-center justify-between border-b px-6 py-3 backdrop-blur-md transition-colors duration-300', t.nav)}>
    ...
    <div className="flex items-center gap-2">
      ...
      <Link to="/login" className={cx('hidden px-3 py-1.5 text-[0.7rem] font-semibold uppercase tracking-wider transition sm:block', t.navText)}>
        Acceder
      </Link>
      <Link
        to="/registro"
...
const HeroSection = ({ t, isDark }) => (
  ...
  >
    <div
        ...
        style={{ backgroundImage: `linear-gradient(${t.gridColor} 1px, transparent 1px), linear-gradient(90deg, ${t.gridColor} 1px, transparent 1px)`, backgroundSize: '48px 48px' }}
      />
    <div className="pointer-events-none absolute inset-0" style={{ background: `radial-gradient(ellipse 80% 70% at -10% 55%, ${t.glowLeft} 0%, transparent 60%)` }} />
    ...
    <div className="relative z-10 flex-1 space-y-7 lg:max-w-2xl">
      ...
      <div data-gsap="hero-badge"
        ...
        <span className="text-[0.58rem] font-bold uppercase tracking-[0.18em]">Para talleres de soldadura · calderería · metal</span>
      </div>

      {/* Pain-first headline */}
      <div className="space-y-3">
        <h1 className="text-[3.8rem] font-black uppercase leading-[0.9] tracking-tighter sm:text-[5rem] lg:text-[5.8rem]">
          <span data-gsap="hero-line" className={cx('block', t.headline)}>¿Sigues</span>
          <span data-gsap="hero-line" className={cx('block', t.headline)}>gestionando</span>
          <span data-gsap="hero-line" className={cx('block', t.headlineAccent)}>con papel</span>
          <span data-gsap="hero-line" className={cx('block', t.headline)}>y Excel?</span>
        </h1>
      ...
      <p data-gsap="hero-sub" className={cx('max-w-lg text-base leading-relaxed', t.muted)}>
        Cada hora que pierdes con papel y llamadas es dinero que no cobras.
        Weldix digitaliza tu taller en 5 minutos — sin instalar nada, sin formación.
      </p>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <a
          ...
          data-gsap="hero-cta"
          className={cx('flex items-center gap-2 rounded-sm px-7 py-3.5 text-sm font-semibold uppercase tracking-wider transition', t.btnWhatsapp)}
        >
      ...
      {/* Stats grid */}
      <div data-gsap="hero-stats" className={cx('grid grid-cols-4 divide-x border', t.divider, isDark ? 'divide-slate-800' : 'divide-stone-200')}>
        {[
          { v: '< 5 min', l: 'setup' },
          { v: '0 €',     l: 'tarjeta' },
          { v: '15 días', l: 'gratis' },
    ...
    {/* Right — mockup */}
    <div data-gsap="hero-mockup" className="relative z-10 mt-14 w-full max-w-md lg:mt-0 lg:max-w-xl">
...
const PainSection = ({ t }) => (
  <section className={cx('border-y px-6 py-16 transition-colors duration-300', t.divider, t.painBg)}>
    <div className="mx-auto max-w-6xl">
      <p className={cx('mb-8 text-center font-mono text-[0.58rem] font-bold uppercase tracking-[0.25em]', t.painText)}>
        // ¿te suena esto?
      </p>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {PAIN_POINTS.map(({ icon, text }) => (
          <div key={text} data-gsap="pain-card" className={cx('flex items-start gap-3 rounded-sm border p-4', t.painBorder)}>
...
const TrustMetricsBar = ({ t, isDark }) => (
  <section className={cx('border-b px-6 py-12 transition-colors duration-300', t.divider, t.sectionAlt)}>
    <div className="mx-auto max-w-6xl">
      <div className={cx('grid grid-cols-2 divide-y lg:grid-cols-4 lg:divide-y-0 lg:divide-x', isDark ? 'divide-slate-800' : 'divide-stone-200')}>
        {TRUST_METRICS.map(({ v, l, icon }) => (
          <div key={l} data-gsap="trust-item" className="flex flex-col items-center gap-2 p-6 text-center">
            <i className={cx(icon, 'text-2xl', t.accent)} />
            <div className={cx('text-2xl font-black', t.accent)}>{v}</div>
            <div className={cx('text-[0.65rem] font-semibold uppercase tracking-[0.12em]', t.muted)}>{l}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  <section className="px-6 py-24">
    <div className="mx-auto max-w-6xl">
      <div className={cx('mb-12 flex flex-col justify-between gap-4 border-b pb-8 sm:flex-row sm:items-end', t.divider)}>
        <div>
          <p className={cx('mb-2 font-mono text-[0.58rem] font-bold uppercase tracking-[0.25em]', t.accent)}>// 01 · módulos</p>
          <h2 className={cx('text-3xl font-black uppercase tracking-tight sm:text-4xl', t.headline)}>Todo lo que necesita<br />tu taller</h2>
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3">
        {FEATURES.map(({ n, icon, title, desc }) => (
          <article key={n} data-gsap="feature-card" className={cx('group relative border-b border-r p-7 transition-colors duration-200 last:border-r-0', t.card, t.cardHover)}>
            <div className="flex items-start gap-4">
              <div className={cx('flex h-10 w-10 shrink-0 items-center justify-center border text-xl', t.accentTint, t.accentBorder, t.accent)}>
                <i className={icon} />
              </div>
  ...
  >
    <div className="mx-auto max-w-6xl">
      <div className={cx('mb-14 border-b pb-8', t.divider)}>
        <p className={cx('mb-2 font-mono text-[0.58rem] font-bold uppercase tracking-[0.25em]', t.accent)}>// 02 · proceso</p>
        <h2 className={cx('text-3xl font-black uppercase tracking-tight sm:text-4xl', t.headline)}>
          De cero a taller digital.<br />Sin complicaciones.
        </h2>
      </div>
      ...
      <div className="relative">
        ...
        <div className={cx('absolute left-0 right-0 top-[2.25rem] hidden h-px lg:block', t.timelineLine)} />
        <div
        ...
        <div className="grid gap-10 lg:grid-cols-4 lg:gap-6">
          {TIMELINE_STEPS.map(({ when, icon, title, desc }, i) => (
            <div key={when} data-gsap="timeline-step" className="relative flex gap-5 lg:flex-col lg:gap-4">
              {/* Dot */}
              <div className={cx('relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2', t.timelineDot)}>
                <i className={cx(icon, 'text-sm text-white')} />
              </div>
              {/* Línea vertical — solo mobile */}
              {i < TIMELINE_STEPS.length - 1 && (
                <div className={cx('absolute left-[1.1rem] top-9 h-full w-px lg:hidden', t.timelineLine)} />
              )}
              <div className="space-y-1 pb-10 lg:pb-0">
                <p className={cx('font-mono text-[0.55rem] font-black uppercase tracking-[0.18em]', t.accent)}>{when}</p>
                <h3 className="text-sm font-black uppercase tracking-wide">{title}</h3>
                <p className={cx('text-xs leading-relaxed', t.muted)}>{desc}</p>
              </div>
            </div>
      ...
      <div className={cx('mt-14 border-t pt-10 text-center', t.divider)}>
        <Link to="/registro" className={cx('inline-flex items-center gap-2 rounded-sm px-8 py-3.5 text-sm font-black uppercase tracking-wider shadow-lg transition', t.btnPrimary)}>
  ...
  <section className="px-6 py-24">
    <div className="mx-auto max-w-6xl">
      <div className={cx('mb-12 border-b pb-8', t.divider)}>
        <p className={cx('mb-2 font-mono text-[0.58rem] font-bold uppercase tracking-[0.25em]', t.accent)}>// 03 · testimonios</p>
        <h2 className={cx('text-3xl font-black uppercase tracking-tight sm:text-4xl', t.headline)}>
          Lo que dicen<br />los talleres
        </h2>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        {TESTIMONIALS.map(({ quote, name, role, initials }) => (
          <blockquote key={name} data-gsap="testimonial" className={cx('relative rounded-sm border p-8 transition-colors duration-200', t.testimonial)}>
            {/* Comillas decorativas */}
            <span className={cx('absolute right-6 top-4 font-serif text-6xl leading-none', t.quoteColor)}>"</span>

            <p className={cx('relative z-10 mb-6 text-sm leading-relaxed', t.muted)}>
              "{quote}"
            </p>

            <footer className="flex items-center gap-3">
              <div className={cx('flex h-10 w-10 shrink-0 items-center justify-center rounded-sm border text-xs font-black', t.accentTint, t.accentBorder, t.accent)}>
                {initials}
              </div>
              <div>
                <p className="text-sm font-bold">{name}</p>
                <p className={cx('text-[0.65rem] font-semibold uppercase tracking-[0.1em]', t.faint)}>{role}</p>
              </div>
            </footer>
          </blockquote>
        ))}
      ...
      <div data-gsap="roi-metric" className={cx('mt-8 rounded-sm border p-6 text-center', t.card)}>
        ...
        <p className={cx('mt-1 text-sm', t.muted)}>de media por taller en los primeros 30 días de uso</p>
      </div>
    </div>
  </section>
)
...
const PricingSection = ({ t }) => (
  <section className={cx('px-6 py-24 transition-colors duration-300', t.sectionAlt)}>
    <div className="mx-auto max-w-6xl">
      <div className={cx('mb-12 flex flex-col justify-between gap-4 border-b pb-8 sm:flex-row sm:items-end', t.divider)}>
        <div>
          <p className={cx('mb-2 font-mono text-[0.58rem] font-bold uppercase tracking-[0.25em]', t.accent)}>// 04 · precios</p>
          <h2 className={cx('text-3xl font-black uppercase tracking-tight sm:text-4xl', t.headline)}>Sin sorpresas.<br />Sin letra pequeña.</h2>
        </div>
        <p className={cx('hidden text-right text-xs', t.muted)}>15 días de prueba gratuita.<br />Sin tarjeta de crédito.</p>
      </div>

      <div className="grid md:grid-cols-3">
        {PLANS.map(({ name, users, price, features, popular }, i) => (
          <div key={name} data-gsap="plan-card" className={cx('relative flex flex-col border p-8 transition-colors', popular ? t.planPopular : t.planNormal, i > 0 && '-ml-px')}>
            ...
            <div className="mb-6 space-y-1">
              ...
              {popular && (
                <span className={cx('inline-flex items-center gap-1 rounded-sm px-2 py-0.5 text-[0.52rem] font-black uppercase tracking-widest', t.accentBg, t.accentBgText)}>
                  <i className="bx bx-star text-[0.7rem]" />Recomendado
                </span>
              )}
            </div>
            <div className="mb-6">
            ...
            <Link to="/registro" className={cx('flex items-center justify-center gap-2 rounded-sm py-3 text-xs font-black uppercase tracking-wider transition', popular ? t.btnPrimary : t.btnGhost)}>
...
const CtaSection = ({ t }) => (
  ...
  >
    <div className="mx-auto max-w-2xl space-y-6">
      <p className={cx('font-mono text-[0.58rem] font-bold uppercase tracking-[0.25em]', t.accent)}>// fin de línea</p>
      <h2 data-gsap="cta-headline" className={cx('text-5xl font-black uppercase leading-tight tracking-tighter sm:text-6xl', t.headline)}>
        Tu taller,<br />
      ...
      <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
        <Link to="/registro"
          ...
          className={cx('flex items-center gap-2 rounded-sm px-10 py-4 text-sm font-black uppercase tracking-wider shadow-lg transition', t.btnPrimary)}>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={cx('flex items-center gap-2 rounded-sm px-8 py-4 text-sm font-semibold uppercase tracking-wider transition', t.btnWhatsapp)}
        >
  ...
  <footer className={cx('border-t px-6 py-8 transition-colors duration-300', t.footerBg, t.divider)}>
    <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 sm:flex-row">
      ...
      <div className={cx('flex items-center gap-6 text-[0.62rem] font-semibold uppercase tracking-[0.12em]', t.faint)}>
        ...
        <a href="mailto:hola@weldix.app" className="transition hover:opacity-100">Contacto</a>
        <Link to="/login" className="transition hover:opacity-100">Acceder</Link>
      </div>
      <p className={cx('font-mono text-[0.52rem] uppercase tracking-widest', t.monoMuted)}>© 2026 Weldix</p>
    </div>
  </footer>
)
