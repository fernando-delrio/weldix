...
const Row = ({ label, value, show = true, green = false }) => (
  ...
  >
    <span className={cx('w-16 shrink-0 text-[0.58rem] uppercase tracking-wider', S.textMuted)}>
      {label}
    </span>
    ...
    <Swap
      ...
      b={
        <div className="space-y-1.5">
          <div
            className={cx(
              'flex items-center gap-2 rounded-sm border border-teal-500/40 bg-teal-500/10 px-5 py-2.5 text-[0.68rem] font-black uppercase tracking-wider',
              S.green
            )}
          >
          ...
          <p className={cx('pl-1 text-[0.6rem]', S.textMuted)}>
            Estado: Pendiente · asignada a Javier Morales
          </p>
        </div>
      }
    />
  ...
  return (
    <div className="space-y-4 px-6 pb-4 pt-2">
      ...
      <div className={cx('space-y-1', vis(step, 1))}>
        <p className={cx('text-[0.8rem] font-bold', S.textStrong)}>
          Soldadura estructura acero inox
        </p>
        <p className={cx('text-[0.68rem]', S.textMuted)}>
          Metalúrgica García S.L. · Javier Morales
        </p>
      </div>
      ...
      <div className={cx('flex items-center justify-between', vis(step, 3))}>
        ...
        <button
          className={cx(
            'rounded-sm border border-yellow-500/30 bg-yellow-500/10 px-3 py-1.5 text-[0.6rem] font-bold uppercase tracking-wider',
            S.accent
          )}
        >
          Pasar a control →
...
const SceneFichaje = ({ step }) => (
  <div className="space-y-4 px-6 pb-4 pt-2">
    ...
    <div className={cx('rounded-sm border p-4 space-y-3', S.border, S.cardBg, vis(step, 1))}>
      ...
      <div
        className={cx('space-y-2 border-t pt-3', S.border, step >= 2 ? 'opacity-100' : 'opacity-0')}
      >
        <div className="flex items-center gap-2">
          <i className={cx('bx bx-shield-check', S.green)} />
          <span className={cx('text-[0.65rem]', S.textMuted)}>
            RDL 8/2019 cumplido · registro geolocalizado
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-teal-400" />
  ...
  <div className="space-y-4 px-6 pb-4 pt-2">
    ...
    <Swap
      ...
      a={
        <div
          className={cx(
            'flex items-center gap-2 rounded-sm border border-teal-500/30 bg-teal-500/10 px-5 py-2.5 text-[0.68rem] font-black uppercase tracking-wider',
            S.green
          )}
        >
      ...
      b={
        <div className="space-y-2">
          <div
            className={cx(
              'flex items-center gap-2 rounded-sm border border-yellow-500/30 bg-yellow-500/10 px-5 py-2.5 text-[0.68rem] font-black uppercase tracking-wider',
              S.accent
            )}
          >
          ...
          <div className={cx('flex items-center gap-1.5 text-[0.62rem]', S.textMuted)}>
            <i className={cx('bx bx-bell text-sm', S.accent)} /> Notificación enviada al admin vía
            n8n
          </div>
        </div>
      }
...
const SceneIA = ({ step }) => (
  <div className="space-y-4 px-6 pb-4 pt-2">
    ...
    <div className={cx('flex items-start gap-3', vis(step, 1))}>
      <span
        className={cx(
          'mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[0.5rem]',
          S.border,
          S.textMuted
        )}
      >
      ...
      <div
        className={cx('rounded-sm rounded-tl-none border px-4 py-2.5', S.border, 'bg-zinc-800/60')}
      >
    ...
    <p
      className={cx(
        'text-[0.65rem] pointer-events-none',
        S.accent,
    ...
    <div className={cx('flex items-start gap-3', vis(step, 3))}>
      <span
        className={cx(
          'mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-yellow-500/40 bg-yellow-500/10 text-[0.5rem]',
          S.accent
        )}
      >
        IA
      ...
      <div className="space-y-1.5 rounded-sm rounded-tl-none border border-yellow-500/20 bg-yellow-500/5 px-4 py-3">
        <p className={cx('text-[0.75rem] font-semibold', S.textStrong)}>
          AWS E308L-16, Ø2.4mm para inox 304/316.
        </p>
        <p className={cx('text-[0.65rem]', S.textMuted)}>
          Tu stock: 6.5 kg disponibles · suficiente para esta OT
        </p>
  ...
  <div className="space-y-4 px-6 pb-4 pt-2">
    ...
    <div className={cx('overflow-hidden rounded-sm border', S.border, S.cardBg, vis(step, 1))}>
      ...
      >
        ...
        <span className={cx('text-[0.68rem]', S.green)}>
          10 días laborables · saldo disponible: 22 días
        </span>
      </div>
    </div>
    <Swap
      ...
      b={
        <div className="space-y-1.5">
          ...
          <p className={cx('pl-1 text-[0.6rem]', S.textMuted)}>
            El admin recibirá notificación en breve
          </p>
        </div>
      }
    />
...
const DemoPreview = ({ t, isDark }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={cx(
          'absolute left-0 top-0 h-6 w-6 border-l-2 border-t-2 pointer-events-none',
          t.accentBorder
        )}
      />
      <div
        className={cx(
          'absolute bottom-0 right-0 h-6 w-6 border-b-2 border-r-2 pointer-events-none',
          t.accentBorder
        )}
      />
      ...
      {/* Cabecera */}
      <div
        className={cx('flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b px-5 py-3', S.border)}
      >
        <span className="h-2 w-2 rounded-full bg-teal-400" />
