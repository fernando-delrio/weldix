...
Migrado desde backend/tests/test_fichaje_service.py — usa la fixture `db`
(SQLite en memoria, sin HTTP) portada a tests/conftest.py (raíz).

IMPORTANTE: test_get_fichajes_operario_scopes_by_tenant es cobertura de
seguridad (IDOR — Insecure Direct Object Reference) y se migra SIN
simplificar: verifica que un mismo operario_id en dos talleres distintos no
cruza datos entre tenants al consultar fichajes.
"""
